...
@torch.no_grad()
def save_mesh(gsNetwork, gsRender, opacity_threshold,density_threshold,  resolution, decimate_target, texture_size, mesh_shape='./outs/mesh/gs_shape.obj', mesh_texture='./outs/mesh/gs_texture.obj', ):
    class MiniCam:
    ...
    def extract_mesh(gsNetwork, density_threshold, resolution, decimate_target):
        @torch.no_grad()
        def extract_fields(gsNetwork, opacity_threshold, resolution, num_blocks=16, relax_ratio=1.5):
            ...
            covs = gsNetwork.covariance_activation(stds, 1, dirs)

            # tile
            occ = torch.zeros([resolution] * 3, dtype=torch.float32, device=opacities.device)
            ...
            for xi, xs in enumerate(X):  #loop blocks (assume max size of gaussian is small than relax_ratio * block_size !!!)
                for yi, ys in enumerate(Y):
                    for zi, zs in enumerate(Z):
                        ...
                        # sample points [M, 3]
                        pts = torch.cat([xx.reshape(-1, 1), yy.reshape(-1, 1), zz.reshape(-1, 1)], dim=-1).to(opacities.device)
        ...
        def clean_mesh(verts, faces, v_pct=1, min_f=64, min_d=20, repair=True, remesh_size=0.01):
            # verts: [N, 3]
            # faces: [N, 3]
            ...
            if remesh_size is not None:
                ...
                ms.meshing_isotropic_explicit_remeshing(iterations=3, targetlen=pml.AbsoluteValue(remesh_size))   #各向通行
            ...
            print(f"cleaning: {_ori_vert_shape} --> {verts.shape}, {_ori_face_shape} --> {faces.shape}")
            return verts, faces
        ...
        def decimate_mesh(verts, faces, target, backend="pymeshlab", remesh=False, optimalplacement=True):
            ...
            print(f"decimation: {_ori_vert_shape} --> {verts.shape}, {_ori_face_shape} --> {faces.shape}")

            return verts, faces

        occ, center, scale = extract_fields(gsNetwork, opacity_threshold, resolution)

        import mcubes
        vertices, triangles = mcubes.marching_cubes(occ.detach().cpu().numpy(), density_threshold)
        vertices = vertices / (resolution - 1.0) * 2 - 1
        vertices = vertices / scale + center.detach().cpu().numpy()  #transform back to the original space

        print(f"marching_cubes: vetext:{list(vertices.shape)}, face:{list(triangles.shape)}")

        Mesh(v=torch.from_numpy(vertices.astype(np.float32)).contiguous().cuda(), f=torch.from_numpy(triangles.astype(np.int32)).contiguous().cuda(), device='cuda').write('./outs/mesh_01.obj')

        vertices, triangles = clean_mesh(vertices, triangles, remesh_size=0.015)
        Mesh(v=torch.from_numpy(vertices.astype(np.float32)).contiguous().cuda(), f=torch.from_numpy(triangles.astype(np.int32)).contiguous().cuda(), device='cuda').write('./outs/mesh_02.obj')
        if decimate_target > 0 and triangles.shape[0] > decimate_target:
            vertices, triangles = decimate_mesh(vertices, triangles, decimate_target)
            Mesh(v=torch.from_numpy(vertices.astype(np.float32)).contiguous().cuda(), f=torch.from_numpy(triangles.astype(np.int32)).contiguous().cuda(), device='cuda').write('./outs/mesh_03.obj')

        vertices = torch.from_numpy(vertices.astype(np.float32)).contiguous().cuda()
        triangles = torch.from_numpy(triangles.astype(np.int32)).contiguous().cuda()
        mesh = Mesh(v=vertices, f=triangles, device='cuda')
        print(f"final: vetext:{list(vertices.shape)}, face:{list(triangles.shape)}, vertex_range:({vertices.min().item()}~{vertices.max().item()})")
    ...
    mesh = extract_mesh(gsNetwork, density_threshold, resolution, decimate_target)
